...
def current_library_search_path():
    current_pass = subprocess.check_output(["pwd"]).decode("utf-8").rstrip('\n')
    os.environ["LD_LIBRARY_PATH"] = f'{current_pass}/bin:{os.environ["LD_LIBRARY_PATH"]}'
    ld_verbose = subprocess.check_output(["./bin/ld.bfd", "--verbose"]).decode("utf-8")
